...
const prompt = ai.definePrompt({
  ...
  **CRITICAL INSTRUCTIONS**:
  ...
  2.  **Prioritize Correlations**: Create a top-level section called '### Key Correlations'. This section is the most important. It must summarize all potential links between symptoms and findings mentioned across ALL the provided reports. Use a separate bullet point for each distinct correlation.
  3.  **Merge Other Sections**: Combine information for other common sections like 'Imaging Findings' and 'Symptom Patterns'. If multiple reports mention imaging, synthesize them into a single 'Imaging Findings' section. Use a separate bullet point for each distinct finding or observation.
  ...
  5.  **Use Bullet Points for Everything**: Every piece of information in every section must be a bullet point. Ensure ample vertical space between sections.
