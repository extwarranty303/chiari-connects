...
const prompt = ai.definePrompt({
  ...
  **CRITICAL INSTRUCTIONS**:
  ...
  5.  **Use Markdown for Readability**: Structure the output with ample spacing. Use headings (e.g., '### Imaging Findings'), bullet points, and bold text.
  ...
  **Analysis Task:**
  ...
  1.  **Imaging Findings**: Under a '### Imaging Findings' heading, describe only the notable anomalies in the image using precise, objective language.
  2.  **Symptom Patterns**: Under a '### Symptom Patterns' heading, highlight the most severe/frequent symptoms.
  3.  **Potential Correlations**: Under a '### Potential Correlations' heading, objectively note potential links between the imaging findings and reported symptoms without claiming causality.
