...
export const categoryDetails: { [key: string]: { name: string; icon: React.ComponentType<LucideProps> } } = categories.reduce((acc, category) => {
    acc[category.slug] = { name: category.name, icon: category.icon };
    return acc;
}, {} as { [key: string]: { name: string; icon: React.ComponentType<LucideProps> } });
