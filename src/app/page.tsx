 ...
 * User Profile, and a conditional Admin Dashboard link.
 *
 * Key functionalities:
 ...
 * - **Role-Based Access**: Shows an "Admin Dashboard" card only to users with admin or moderator roles.
...
export default function MainPage() {
  const { user, userProfile, isUserLoading, isAdmin, isModerator } = useUser();
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {isPrivilegedUser && (
                 <Link href="/admin">
                    <Card className="glassmorphism hover:border-destructive/50 transition-all duration-200 group flex flex-col h-full border-destructive/20">
                    <CardHeader>
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                                ...
                                <CardDescription className="text-foreground/80 mt-1">Manage users, moderate content, and view site-wide reports.</CardDescription>
                            </div>
                        </div>
                    </CardHeader>
                    <CardContent className="mt-auto">
                        <span className="text-sm font-semibold text-destructive flex items-center gap-2">
                            Go to Dashboard <ArrowRight className="h-4 w-4 opacity-0 group-hover:opacity-100 transition-opacity" />
                        </span>
                    </CardContent>
                    </Card>
                </Link>
